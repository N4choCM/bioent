	...
	const navItems = [
		{
			to: "/",
			label: "Inicio",
			icon: <Home20Regular className="mobile-bigger" />,
			onClick: closeMenu,
		},
		{
			to: "/about",
			label: "Sobre mí",
			icon: <BookContacts20Regular className="mobile-bigger" />,
			onClick: closeMenu,
		},
		{
			label: "Servicios",
			...
			subItems: [
				{
					to: "/services/fitness",
					label: "Entrenamientos Fitness",
					icon: <DirectionsRunIcon className="mobile-bigger mui-icon me-1" />,
					onClick: closeMenu,
				},
				{
					to: "/services/personal-training",
					label: "Entrenamientos Personales",
					icon: <DirectionsBikeIcon className="mobile-bigger mui-icon me-1" />,
					onClick: closeMenu,
				},
				{
					to: "/services/biomechanics",
					label: "Biomecánica",
					icon: <ConstructionIcon className="mobile-bigger mui-icon me-1" />,
					onClick: closeMenu,
				},
			],
		},
		{
			to: "/contact",
			label: "Contacto",
			icon: <EmailIcon className="mobile-bigger mui-icon me-1" />,
			onClick: closeMenu,
		},
	];
					...
					>
						<ul className="navbar-nav ms-auto">
							{navItems.map((item, index) => (
								<li
									key={index}
									className={`nav-item navbar-hover-custom text-white me-1 navbar-flex ${
						...
						<div className="d-flex navbar-hover-custom navbar-flex">
							<button className="btn" onClick={handleDarkModeClick}>
								{appStateContext?.state.isDarkMode ? (
